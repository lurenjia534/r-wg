...
async fn dns_backend_order(info: &ResolvConfInfo) -> Vec<DnsBackendKind> {
    ...
    let order =
        dns_backend_order_from_probes(info, preferred, resolved_ok, resolvconf_ok, nm_ok);
    let _ = DNS_BACKEND_ORDER_CACHE.set(order.clone());
    order
}

fn dns_backend_order_from_probes(
    info: &ResolvConfInfo,
...
mod tests {
    ...
    fn resolv_conf_info(is_symlink: bool, target: Option<&str>, contents: Option<&str>) -> ResolvConfInfo {
        ResolvConfInfo {
    ...
    fn backend_order_keeps_resolv_conf_as_regular_file_fallback() {
        ...
        let order = dns_backend_order_from_probes(&info, detect_preferred_backend(&info), false, true, false);
