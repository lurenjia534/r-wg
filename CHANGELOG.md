...
- Reworked runtime logging around a unified tracing pipeline that captures `r_wg::*` targets, maps UI/backend/network/DNS/IPC/service scopes consistently, and keeps buffered and stderr output under one configuration model.
- Added privileged backend log snapshot and clear commands over IPC v12, allowing the Logs page to merge UI-process logs with backend service logs while avoiding continuous polling when the page is not active.
- Added a persisted Preferences -> Monitoring -> Logs switch for enabling or disabling the UI log viewer, with Copy, Clear, and Auto Follow behavior aligned to the viewer state.
- Updated the logging documentation to match the implemented behavior, including process-local buffers, target-to-scope mapping, environment variable semantics, backend sync limits, and release compatibility notes.
